...
const Nav = () => {
  ...
  const handleDropDownKey = event => {
    if (inputValue) {
      if (
        event.key === 'ArrowDown' &&
        allBooksData.length - 1 > dropDownItemIndex
      ) {
      ...
      if (event.key === 'Enter' && dropDownItemIndex >= 0) {
        clickDropDownItem(allBooksData[dropDownItemIndex]);
    ...
    <Wrapper>
      ...
      <SearchSection>
        <WholeBox>
          ...
          {inputValue && (
            <DropDownBox>
              {chosenTextList.length === 0 && (
                <DropDownItem>해당하는 만화가 없습니다</DropDownItem>
              )}
              {chosenTextList.map((dropDownItem, dropDownIndex) => {
                return (
                  <DropDownItem
                    key={dropDownIndex}
